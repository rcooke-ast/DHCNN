...
from utils import generate_fakespectra, load_atomic
...
def extraction(zem, numseg, numlym, mock_spec, HI_comps, numNzb_all):
    ...
    numNzb = max(numNzb_all,
                 zarr.size // numsamp)  # The maximum number of lines in any segment is guessed to be the average
...
def cnn_numabs(zem=3.0, numseg=512, numspec=1, seed=None, snr=30, plotsegs=False):
    ...
    for dd in range(numspec):
        ...
        numsamp = npix // numseg  # Total number of samples to generate
        numNzb = max(numNzb_all,
                     wz[0].size // numsamp)  # The maximum number of lines in any segment is guessed to be the average
        ...
        for ss, snr in enumerate(snrs):
            ...
            if dd == 0:
                ...
                if ss == 0:
                    ...
                    Ndata_all = [-1 * np.ones((numspec, numNzb), dtype=np.float) for all in
                                 snrs]  # Labels (column density)
                    bdata_all = [-1 * np.ones((numspec, numNzb), dtype=np.float) for all in
                                 snrs]  # Labels (Doppler parameter)
                # Fill
                ...
                if numNzb_all[ss] < zshp:
                    zdata_all[ss] = np.pad(zdata_all[ss], ((0, 0), (0, zshp - numNzb_all[ss])), 'constant',
                                           constant_values=-1)
                    Ndata_all[ss] = np.pad(Ndata_all[ss], ((0, 0), (0, zshp - numNzb_all[ss])), 'constant',
                                           constant_values=-1)
                    bdata_all[ss] = np.pad(bdata_all[ss], ((0, 0), (0, zshp - numNzb_all[ss])), 'constant',
                                           constant_values=-1)
                    # Update the max array size
                    numNzb_all[ss] = zshp
            ...
            for snr in snrs:
                ...
                for jj in range(nruns):
                    ...
                    async_results.append(
                        pool.apply_async(cnn_lyman_images, (zem, numwav, numlym, numspec, seed, snr, jj)))
                pool.close()
                pool.join()
                map(ApplyResult.wait, async_results)
