...
def voigt(par, wavein, logn=True):
    epar = [par[3] * 1.0e-8, 0.4164, 6.265E8]  # Note, the wavelength is converted to cm
    # Column density
    if logn:
        cold = 10.0 ** par[0]
    else:
...
def DH_model(par, wavein):
    ...
    bval = np.sqrt(dopp**2 + 0.01662892444*temp/1.00782503207)
    wcen = 1215.6701
    model *= voigt([NHI, zabs, bval, wcen], wavein)
    # Generate D I profile
